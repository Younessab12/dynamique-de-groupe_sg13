...
const firstSubmit =  async(code)=>{
    const res = await axios.get('http://localhost:8080/users/api/submited_codes');
        const submited = []
      const headerDate =   res.headers && res.headers.date ? res.headers.date : 'no response date';
      const users =  res.data;
      for( const user in users){
          submited.push(users[user][0])
      }
      if(submited.indexOf(code)==-1 ){
        return true;
    }
    else{
        return false;
...
router.post('/post', async(req,res)=>{
    ...
    let check = await firstSubmit(code);
   ...
   if(isValidCode(code)){
        if(check){
            ...
            res.status(400).json({message : "this code has already voted "})
        }
   }
   else {
